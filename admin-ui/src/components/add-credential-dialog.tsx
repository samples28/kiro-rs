...
export function AddCredentialDialog({ open, onOpenChange, onBalanceUpdate }: AddCredentialDialogProps) {
  const savedProxy = storage.getProxySettings()
  ...
  const [proxyUsername, setProxyUsername] = useState(savedProxy?.proxyUsername ?? '')
  const [proxyPassword, setProxyPassword] = useState(savedProxy?.proxyPassword ?? '')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutate(
      ...
      {
        onSuccess: (data) => {
          storage.setProxySettings({
            proxyUrl: proxyUrl.trim(),
            proxyUsername: proxyUsername.trim(),
            proxyPassword: proxyPassword.trim(),
          })
          if (data.balance && onBalanceUpdate) {
            onBalanceUpdate(data.credentialId, data.balance)
          }
