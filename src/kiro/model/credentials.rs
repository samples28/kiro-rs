...
pub struct KiroCredentials {
    ...
    /// 凭据级冷却限流开关（独立于全局设置）
    /// Some(true) = 使用凭据自身的冷却参数
    /// Some(false) = 该凭据不限流（即使全局开启）
    /// None = 跟随全局设置
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cooldown_enabled: Option<bool>,

    /// 凭据级冷却窗口时长（秒），未设置时回退到全局 cooldownSeconds
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cooldown_seconds: Option<u64>,

    /// 凭据级冷却窗口内最大请求数，未设置时回退到全局 cooldownMaxRequests
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KiroCredentials {
    ...
    /// 检查凭据是否支持指定模型
    ///
    /// Free 账号只支持 free_model_mappings 中配置的模型（from 字段匹配）
    pub fn supports_model(&self, model: &str, free_model_mappings: &[crate::model::config::ModelMapping]) -> bool {
        if !self.is_free() {
            return true;
        }
        let m = model.to_lowercase();
        // free_model_mappings 中的 from 字段作为关键字匹配
        free_model_mappings.iter().any(|mapping| m.contains(&mapping.from.to_lowercase()))
    }
